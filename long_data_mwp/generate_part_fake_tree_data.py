    ...
    for key, value in paths.items():
        ...
        for i, f in enumerate(value[1]):
            if f > 0:
                ...
                feature_list.append(f'(f{feature_part[0] + i + 1} <= 1.000)')
...
def min_max_norm(data):
    ...
    for i, d in enumerate(data):
        for f in range(len(d)):
            norm_data[i, f] = (d[f] - min_data[f]) / (max_data[f] - min_data[f])
...
def get_all_tree_paths(tree, feature_part):
    ...
    for p in paths.keys():
        ...
        for f in paths[p][0]:
            ...
            rule_of_each_f[int(ff) - feature_part[0] - 1] += 1
...
def train_DT(data, target, fake_data, feature_part):
...
def generate_data(data_name, data, target, fake_data):
    all_rule = []
    for feature_part in cutting_feature(feature_num, max_depth):
        rule_t, rule_f = train_DT(data, target, fake_data, feature_part)
...
max_depth = 5
data_name = f'part_fake_german_tree_md{max_depth}_float{3}'
fake_data = np.round(np.random.uniform(-1, 1, size = (10000, feature_num)), 3)

generate_data(data_name, real_x, real_y, fake_data)