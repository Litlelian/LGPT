    ...
    for x, label in tqdm(tt_set):                       
        with torch.no_grad():                   
            ...
            for p in range(len(x)):
                ...
                ans.append(model.tokenizer(label[p])['input_ids'][mask_index])
                if torch.argmax(pred['logits'][p, mask_index]).detach().cpu().item() == model.tokenizer(label[p])['input_ids'][mask_index]:
                    acc += 1
    test_acc = acc / len(tt_set.dataset)
    return test_acc, preds, ans
...
if __name__ == "__main__":
    ...
    data_name = 'tree_r3_f3_float3'

    test_data, test_label = read_data('dataset/' + data_name + '/' + data_name + '_test')
    # test_data, test_label = read_data('dataset/part_german_tree_md3_float3')
    ...
    LM.load_state_dict(torch.load(f'models/tree_r{r_f}_f{r_f}_float3.pth'))

    test_acc, preds, ans = test(tt_set, LM)
    print(f'acc = {test_acc}')

    # Confusion Matrix
    cm = confusion_matrix(ans, preds, labels=[1606, 1607])
    # cm = confusion_matrix(ans, preds, labels=[2995, 6270])
    print(cm)
    print(ans[:10])
    print(ans[-10:])