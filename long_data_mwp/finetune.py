...
def train(tr_set, dv_set, model):
    ...
    while (epoch < max_epoch) and (early_stopping_count<early_stopping_step):
        ...
        if dev_loss < min_loss:
            ...
            torch.save(model.state_dict(), 'models/' + data_name + '.pth')
        else:
...
if __name__ == "__main__":
    ...
    train_data, train_label = read_data('dataset/' + data_name + '/' + data_name + '_train')
    dev_data, dev_label = read_data('dataset/' + data_name + '/' + data_name + '_dev')  # rule not in train
    ...
    model_epoch, model_record = train(tr_set, dv_set, LM)

    with open("record/" + data_name + ".pkl", "wb") as tf:
        pickle.dump(model_record, tf)

    LM.load_state_dict(torch.load("models/" + data_name + ".pth"))
